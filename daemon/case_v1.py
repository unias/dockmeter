...
class case_handler:
	...
	# curl -X POST http://0.0.0.0:1726/v1/system/loads
	def system_loads(form):
		return system_manager.get_system_loads()
	
	# curl -X POST http://0.0.0.0:1726/v1/system/memsw/available
	def system_memsw_available(form):
		return system_manager.get_available_memsw()
	
	# curl -X POST -F size=20 http://0.0.0.0:1726/v1/system/swap/resize
